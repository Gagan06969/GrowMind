...
const Analytics = ({ trees = [], sessions = [] }) => {
  ...
  const categories = ['DSA', 'Aptitude', 'Core Subjects', 'Projects', 'Misc'];
  const categoryData = categories.map(cat => {
    const catSessions = sessions.filter(s => s.category === cat);
    const totalHours = catSessions.reduce((acc, s) => acc + (s.duration / 3600), 0);
    // Scale hours to a "mastery" percentage (e.g., 10 hours = 100%)
    const mastery = Math.min(100, (totalHours / 10) * 100);
    return { subject: cat, value: parseFloat(mastery.toFixed(0)), fullMark: 100 };
  });

  // 3. Consistency Score (Days focused in last 7 days)
  ...
  const nextMilestone = milestones.find(m => m > trees.length) || 100;
  const treesNeeded = Math.max(0, nextMilestone - trees.length);
    ...
    <div className="analytics-container" style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
        ...
        <div className="card glass-morphism" style={{ padding: '24px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '36px', fontWeight: 'bold', color: '#e67e22', margin: '10px 0' }}>{treesNeeded} Trees</div>
          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>To reach {nextMilestone} tree milestone</div>
        </div>
      </div>
    </div>
  );
